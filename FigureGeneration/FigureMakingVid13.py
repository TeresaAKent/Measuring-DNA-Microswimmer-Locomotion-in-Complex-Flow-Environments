...
# Give an output name
OutputfileName = "TarynPutLabelsAboutDataHere"

## Load Fiducials
Locations = "MagPull_MS13_Fiducials.xlsx"
...
##Load Swimmer Data
Locations = "MagPull_MS13_Position.xlsx"
...
Locations = "MagPull_MS13_MagFid.xlsx"
...
Locations1 = "MagPull_MS13_Angle.xlsx"
...
# This Motion is a relative motion
MotionM = np.array(FinalDataM[:EndPoint,1:]-FinalDataM[1,1:], dtype = float)
# Break motion in to magnitudes directions and x and y vectors
MotionMX = np.reshape(MotionM[:,0::2],(-1,int(np.size(MotionM,1)/2)))
MotionMY = np.reshape(MotionM[:,1::2],(-1,int(np.size(MotionM,1)/2)))
...
# This Motion is a relative motion
MotionMag = np.array(FinalDataMag[:EndPoint,1:]-FinalDataMag[1,1:], dtype = float)
# Break motion in to magnitudes directions and x and y vectors
...
for time in range(EndPoint):
    NotTooCloseToSwimmer = np.argwhere(DistTot[time,:]>250)
    NotTooCloseToMagnet = np.argwhere(DistTotMF[time,:]>250)
...
plt.plot(FinalData[:EndPoint,0],MotionMX,'k')
#plt.ylim([-15,35])
plt.xlabel('Time [ms]')
...
plt.subplot(5,1,2)
Regs = np.mean(np.sqrt(MotionY[:,0:1]**2+MotionX[:,0:1]**2),1)
...
#Dir[Dir<-90]=Dir[Dir<-90]+360
#ax6=axes[3].twinx()
axes[3].plot(FinalData[:707,0],Mags[:707,0]/convert,color = [.443,0,.467])
axes[3].plot(FinalData[707:EndPoint,0].flatten(),Mags[707:,0]/convert,color = [165/255,23/255,167/255])
#axes[1].fill_between(np.linspace(0,87683.2814/1000,EndPoint), (Regs-RegStd).flatten()/convert, (Regs+RegStd).flatten()/convert, color = [83/250,19/250,211/250], alpha=.75)
...
# Load the vide
videoPath = "MagPull_M13_constDirRotSine_trim.mp4"
...
OutputfileName = "Vid13ResultsVideo"
MovVideoOutputName = "{}MovingFrame.mp4".format(OutputfileName)
video=cv2.VideoWriter(MovVideoOutputName,fourcc,framesPerSecond/divisor,(int(np.size(FramesToVideo,1)+abs(MaxMoveX)+35),int(abs(MaxMoveY)+35+np.size(FramesToVideo,0))))
...
for j in range(int(np.size(XNetX,0)/divisor)+1):
    ...
    for c in range(np.size(MotionMX,1)):
        cv2.circle(ArrowFrame,(int(FinalDataM[0,1+2*c]+MotionMagX[i,0]),np.size(ArrowFrame,0)-(int(FinalDataM[0,2+2*c]+MotionMagY[i,0]))),15,(255,0,255),2)
...
plt.subplot(3,1,2)
# plt.scatter(MotionX[:707,0],MotionY[:707,0],.5,color='b')
# plt.scatter(MotionX[707:,0],MotionY[707:,0],.5,color='b')
XAverage=np.median(MotionX,1)
YAverage=np.median(MotionY,1)
plt.plot(XAverage,YAverage,.5,color='b')
STDEVX=(np.std(MotionX,1))
STDEVY=np.std(MotionY,1)
SummarySTDEV=np.sqrt(np.square(STDEVX)+np.square(STDEVY))
plt.fill_between(XAverage, (YAverage-SummarySTDEV), (YAverage+SummarySTDEV), color=[.25,.25,1], alpha=.5)

plt.scatter(MotionMagX[:707,0],MotionMagY[:707,0],.5,color='r')
plt.scatter(MotionMagX[707:,0],MotionMagY[707:,0],.5,color='r')
plt.scatter(MotionMX[:707,1],MotionMY[:707,1],.5,color=[.443,0,.467])
plt.scatter(MotionMX[707:,1],MotionMY[707:,1],.5,color=[206.25/255,28.75/255,208.75/255])
plt.ylabel("$\Delta$ y")
plt.xlabel("$\Delta$ x")
...
import scipy
Error = np.sqrt((MotionMY[:EndPoint,1].flatten()-MotionMagY[:EndPoint].flatten())**2+(MotionMX[:EndPoint,1].flatten()-MotionMagX[:EndPoint].flatten())**2)
slope, intercept, r_value, p_value, std_err = scipy.stats.linregress(FinalData[:EndPoint,0].astype(float)/1000, Error.astype(float))
#Error = (Ms[:EndPoint,1].flatten()-Mags[:EndPoint].flatten())
Dir = np.arctan2(MotionMY-np.roll(MotionMY,10),MotionMX-np.roll(MotionMX,10))*180/3.14159
#Dir[Dir<-90]=Dir[Dir<-90]+360
#ax6=axes[3].twinx()
plt.plot(FinalData[:700,0]/1000,abs(Error[:700])/convert,color = [.443,0,.467])
plt.plot(FinalData[700:EndPoint,0]/1000,abs(Error[700:EndPoint])/convert,color = [206.25/255,28.75/255,208.75/255])
#axes[1].fill_between(np.linspace(0,87683.2814/1000,EndPoint), (Regs-RegStd).flatten()/convert, (Regs+RegStd).flatten()/convert, color = [83/250,19/250,211/250], alpha=.75)
plt.xlabel('Time [s]')
plt.ylabel('$\Delta$ [$\mu$m]',color = [.443,0,.467])
plt.tick_params(axis="y", labelcolor=[.443,0,.467])
...
plt.subplot(3,1,1)
vertical_img = np.flip(FramesToVideo[:,:,:,400],0)
plt.imshow(vertical_img, origin= 'lower')
MagX=np.average(MotionMagX,1)
MagY=np.average(MotionMagY,1)
plt.scatter(FinalData[1,1::2]+FNetX[2000,:],FinalData[1,2::2]+FNetY[2000,:], 50, color = 'b', alpha = 0.25)
plt.scatter(FinalDataMag[1,1::2]+MagX[2000],FinalDataMag[1,2::2]+MagY[2000], 50, color = 'r', alpha = 0.25)
plt.scatter(FinalDataM[1,1::2]+MagX[2000],FinalDataM[1,2::2]+MagY[2000], 50, color = [165/255,23/255,167/255], alpha = 0.25)
plt.axis('off')
