...
Locations1 = "TAEPull_MS10_Angle.xlsx"
...
Motion = np.array(FinalData[:EndPoint,1:]-FinalData[1,1:], dtype= float)
...
# This Motion is a relative motion
MotionM = np.array(FinalDataM[:EndPoint,1:]-FinalDataM[20,1:], dtype = float)
# Break motion in to magnitudes directions and x and y vectors
...
for time in range(EndPoint):
    NotTooCloseToSwimmer = np.argwhere(DistTot[time,:]>200)
    NotTooCloseToMagnet = np.argwhere(DistTotMF[time,:]>200)
...
plt.subplot(5,1,1)
plt.plot(FieldData[:,0],FieldData[:,1],'k')
plt.xlabel('Time [ms]')
...
#ax2=axes[1].twinx()
axes[1].plot(FinalData[:EndPoint,0],Regs/convert,color = [83/250,19/250,211/250])
axes[1].fill_between(np.linspace(0,97.3607,EndPoint), (Regs-RegStd).flatten()/convert, (Regs+RegStd).flatten()/convert, color = [83/250,19/250,211/250], alpha=.75)
axes[1].set_xlabel('Time [s]')
axes[1].set_ylabel('$\Delta$ [$\mu$m]',color = [83/250,19/250,211/250])
axes[1].tick_params(axis="y", labelcolor=[83/250,19/250,211/250])
plt.ylim([0,75])
...
#ax4=axes[2].twinx()
axes[2].plot(FinalData[:EndPoint,0],Mags/convert,color = [240/250,8/250,0/250])
...
axes[2].set_ylabel('$\Delta$ [$\mu$m]',color = [240/250,8/250,0/250])
axes[2].tick_params(axis="y", labelcolor=[240/250,8/250,0/250])
plt.ylim([0,75])
...
plt.subplot(5,1,4)
Mags = np.sqrt(MotionMY**2+MotionMX**2)
Dir = np.arctan2(MotionMY-np.roll(MotionMY,10),MotionMX-np.roll(MotionMX,10))*180/3.14159
#Dir[Dir<-90]=Dir[Dir<-90]+360
#ax6=axes[3].twinx()
axes[3].plot(FinalData[:707,0],Mags[:707]/convert,color = [.443,0,.467])
axes[3].plot(FinalData[707:EndPoint,0].flatten(),Mags[707:]/convert,color = [165/255,23/255,167/255])
#axes[1].fill_between(np.linspace(0,87683.2814/1000,EndPoint), (Regs-RegStd).flatten()/convert, (Regs+RegStd).flatten()/convert, color = [83/250,19/250,211/250], alpha=.75)
axes[3].set_xlabel('Time [s]')
axes[3].set_ylabel('$\Delta$ [$\mu$m]',color = [.443,0,.467])
axes[3].tick_params(axis="y", labelcolor=[.443,0,.467])
plt.ylim([0,75])

plt.subplot(5,1,5)
plt.plot(SwimmerOrientation[:,0],SwimmerOrientation[:,1],color = [.443,0,.467])
plt.xlabel('Time [ms]')
axes[4].tick_params(axis="y", labelcolor=[.443,0,.467])
axes[4].set_ylabel('Swimmer\n Orientation [$^\circ$]',color = [.443,0,.467])
...
for j in range(int(np.size(XNetX,0)/divisor)+1):
    ...
    for l in range(np.size(MotionMagX,1)):
        cv2.circle(ArrowFrame,(int(FinalDataMag[0,1+l*2]+MotionMagX[i,0]),np.size(ArrowFrame,0)-(int(FinalDataMag[0,2+2*l]+MotionMagY[i,0]))),15,(0,0,255),2)
...
ax2=axes[0].twinx()
axes[0].plot(FieldData[:EndPoint,0]/1000,Error, 1, color =  [0/250,0/250,250/250])
axes[0].set_ylabel("$\Delta_{PS,i}-\Delta_{PS}$ [$\mu$m]", color =[0/250,0/250,250/250])
axes[0].set_xlabel("Time [s]")
axes[0].tick_params(axis="y", labelcolor=[83/250,19/250,211/250])
ax2.plot(FieldData[:EndPoint,0]/1000,DistTot[:EndPoint,ii]/convert, 1, color = 'k')
ax2.set_ylabel('Distance from Swimmer [$\mu$m]',color ='k')
ax2.tick_params(axis="y", labelcolor="k")
ax2.axhline(y=200/convert, color=[0.5,0.5,0.5], linestyle='--')
ax2.set_ylim([0,270])
...
PercentError = Error/TotalDist
ax4=axes[1].twinx()
axes[1].plot(FieldData[:EndPoint,0]/1000,Error, 1, color =  [0/250,0/250,250/250])
axes[1].set_ylabel("$\Delta_{PS,i}-\Delta_{PS}$ [$\mu$m]", color =[0/250,0/250,250/250])
axes[1].set_xlabel("Time [s]")
ax4.set_ylim([0,270])
ax4.plot(FieldData[:EndPoint,0]/1000,DistTot[:EndPoint,ii]/convert, 1, color = 'k')
ax4.set_ylabel('Distance from Swimmer [$\mu$m]',color ='k')
ax4.tick_params(axis="y", labelcolor="k")
ax4.axhline(y=200/convert, color=[0.5,0.5,0.5], linestyle='--')
...
for ii in range (np.size(MotionX,1)):
    ...
    if ii==2:
        ...
        plt.arrow(FinalData[1750,1+ii*2],FinalData[1750,2+ii*2],FNetX[1750,ii]-MotionX[1750,ii], FNetY[1750,ii]-MotionY[1750,ii], width=4, color = [0/250,0/250,250/250],alpha =0.75, hatch='X',length_includes_head=True)
plt.plot(circleX+FinalDataM[1750,1],circleY+FinalDataM[1750,2],color=[0.5,0.5,0.5])    
...
#Dir[Dir<-90]=Dir[Dir<-90]+360
#ax6=axes[3].twinx()
plt.plot(FinalData[:707,0],Ms[:707]/convert-Mags[:707]/convert,color = [.443,0,.467])
plt.plot(FinalData[707:EndPoint,0].flatten(),Ms[707:]/convert-Mags[707:]/convert,color = [165/255,23/255,167/255])
#axes[1].fill_between(np.linspace(0,87683.2814/1000,EndPoint), (Regs-RegStd).flatten()/convert, (Regs+RegStd).flatten()/convert, color = [83/250,19/250,211/250], alpha=.75)
plt.xlabel('Time [s]')
...
plt.subplot(2,2,3)
plt.plot(FieldData[:EndPoint,0]/1000,FieldData[:EndPoint,1],'k')
plt.xlabel('Time [ms]')
plt.ylabel('Magnetic Field \n Orientation [$^\circ$]')
...
plt.scatter(MotionMX[707:,0],MotionMY[707:,0],.5,color=[165/255,23/255,167/255])
plt.ylabel("$\Delta$ y")
plt.xlabel("$\Delta$ x")
...
STDEVX=(np.std(MotionX[:,[2,3,4,7,8,9,10]],1))
STDEVY=np.std(MotionY[:,[2,3,4,7,8,9,10]],1)
...
plt.fill_between(np.linspace(np.min(XAverage),np.max(XAverage),np.size(XAverage)), (YAverage-SummarySTDEV), (YAverage+SummarySTDEV), color=[.25,.25,1], alpha=.5)
...
plt.fill_between(np.linspace(np.min(MagXAverage),np.max(MagXAverage),np.size(MagXAverage)), MagYAverage-SummarySTDEV, MagYAverage+SummarySTDEV, color=[1,.25,.25], alpha=.5)
plt.plot(MotionMX[:707,0],MotionMY[:707,0],.5,color=[.443,0,.467])
plt.plot(MotionMX[707:,0],MotionMY[707:,0],.5,color=[206.25/255,28.75/255,208.75/255])
plt.ylabel("$\Delta$ y")
plt.xlabel("$\Delta$ x")
...
Ms = np.sqrt(MotionMY**2+MotionMX**2)
Mags = np.sqrt(MotionMagY**2+MotionMagX**2)
Error = Ms.T-Mags[:EndPoint,1]
Error = np.sqrt((MotionMY[:EndPoint,0].flatten()-MotionMagY[:EndPoint,1].flatten())**2+(MotionMX[:EndPoint,0].flatten()-MotionMagX[:EndPoint,1].flatten())**2)
Dir = np.arctan2(MotionMY-np.roll(MotionMY,10),MotionMX-np.roll(MotionMX,10))*180/3.14159
#Dir[Dir<-90]=Dir[Dir<-90]+360
#ax6=axes[3].twinx()

#Diff2 = np.sqrt((MotionMY[:,1].flatten()-MotionMagY.flatten())**2+(MotionMX[:,1].flatten()-MotionMagX.flatten())**2)

plt.plot(FinalData[:700,0]/1000,Error[:700]/convert,color = [.443,0,.467])
plt.plot(FinalData[700:EndPoint,0]/1000,Error[700:EndPoint]/convert,color = [206.25/255,28.75/255,208.75/255])
#axes[1].fill_between(np.linspace(0,87683.2814/1000,EndPoint), (Regs-RegStd).flatten()/convert, (Regs+RegStd).flatten()/convert, color = [83/250,19/250,211/250], alpha=.75)
plt.xlabel('Time [s]')
plt.ylabel('$\Delta$ [$\mu$m]',color = [.443,0,.467])
plt.tick_params(axis="y", labelcolor=[.443,0,.467])
...
vertical_img = np.flip(FramesToVideo[:,:,:,350],0)
plt.imshow(vertical_img, origin= 'lower')
plt.scatter(FinalData[1,1::2]+FNetX[1750,:],FinalData[1,2::2]+FNetY[1750,:], 50, color = 'b', alpha = 0.25)
plt.scatter(FinalDataMag[1,1::2]+np.median(MotionMagX[1750,:],0),FinalDataMag[1,2::2]+np.median(MotionMagY[1750,:],0), 50, color = 'r', alpha = 0.25)
plt.scatter(FinalDataM[1,1::2]+np.median(MotionMagX[1750,:]),FinalDataM[1,2::2]+np.median(MotionMagY[1750,:]), 50, color = [165/255,23/255,167/255], alpha = 0.25)
plt.axis('off')
